...
public class BLFacadeImplementation implements BLFacade {
    ...
    public Socio login(String user, String pass) {
        ...
        if (socio != null && socio.getPass().equals(pass)) {
            Socio respuesta = new Socio();
            respuesta.setUser(socio.getUser());
            respuesta.setPass(socio.getPass());
            respuesta.setEmail(socio.getEmail());
            respuesta.setCuenta(socio.getCuenta());
            respuesta.setMaxSemana(socio.getMaxSemana());
            respuesta.setSemana(socio.getSemana());
            respuesta.setAdmin(socio.isAdmin());
            return respuesta;
        }
            ...
            for (Socio socio : socios) {
                ...
                if (facturables.size() > 4) {
                    ...
                    if (f.getTotal() > 0) {
                        ...
                        nuevas.add(f);
                        System.out.println("factura creada para " + sDB.getUser());
                    }
                }
    ...
    public List<Factura> verFacturas(Socio socio) {
        try {
            dbManager.open();
            List<Factura> originales = dbManager.getFacturasDeSocioPorUser(socio.getUser());
            dbManager.close();
